    ...
    GLuint LoadDds(const char* FilePath, int& Width, int& Height)
    {
        ...
        switch (headerDxt.dxgiFormat)
        {
            ...
            case DXGI_FORMAT_BC7_UNORM: // BPTC
                ...
                format = GL_NONE;
                break;
            case DXGI_FORMAT_BC6H_UF16: // HDR compressed (unsigned float)
                internalFormat = GL_COMPRESSED_RGB_BPTC_UNSIGNED_FLOAT;
                blockSize = 16;
                dataType = GL_NONE;
                format = GL_NONE;
                break;
            case DXGI_FORMAT_BC6H_SF16: // HDR compressed (signed float)
        ...
        if (format != GL_NONE)
        {
            for (unsigned int i = 0; i < mipMapCount; i++)
            {
                ...
                size = w * h * blockSize;
                glTexImage2D(GL_TEXTURE_2D, static_cast<GLint>(i), internalFormat, w, h, 0, format, dataType,
                             buffer + offset);
                offset += size;
                w /= 2;
                h /= 2;
        ...
        else
        {
            for (unsigned int i = 0; i < mipMapCount; i++)
            {
                ...
                glCompressedTexImage2D(GL_TEXTURE_2D, static_cast<int32_t>(i), internalFormat, w, h, 0,
                                       size, buffer + offset);
                offset += size;
                w /= 2;
                h /= 2;
