...
void Engine::VacuumShotVfx::Render(const Engine::CameraRenderData& RenderData)
{
...
void Engine::VacuumShotVfx::Start()
{
...
void Engine::VacuumShotVfx::DispatchSpawnShaders(const float DeltaTime)
{
    if (!ShouldSpawn)
    {
        return;
    }
    constexpr int particlesToSpawn = 16;
    ShouldSpawn = false;
    ...
    constexpr int workGroupsCount = (particlesToSpawn + workGroupSize - 1) / workGroupSize;
...
void Engine::VacuumShotVfx::DispatchUpdateShaders(const float DeltaTime)
{
    UpdateShader.Use();
...
#if EDITOR
void Engine::VacuumShotVfx::DrawImGui()
{
    if (ImGui::CollapsingHeader("Vacuum Shot VFX"))
    {
    }
}
#endif
rapidjson::Value Engine::VacuumShotVfx::Serialize(rapidjson::Document::AllocatorType& Allocator) const
{
    START_COMPONENT_SERIALIZATION
    END_COMPONENT_SERIALIZATION
}

void Engine::VacuumShotVfx::DeserializeValuePass(const rapidjson::Value& Object,
...
void Engine::VacuumShotVfx::DeserializeReferencesPass(const rapidjson::Value& Object,
                                                      Serialization::ReferenceTable& ReferenceMap)
{
